...
public class CartItemConverter implements Converter {
	...
	public IModel toModel(IDto dtoObject) {
		...
		if(productTypeOfTheProduct.equals("CPU")) {
			cartItem.setProduct((CPU) cpuConverter.toModel(cpuService.getCpuById(cartItem.getProduct().getIdProduct())));
		} else if(productTypeOfTheProduct.equals("GPU")) {
			cartItem.setProduct((GPU) gpuConverter.toModel(gpuService.getGpuById(cartItem.getProduct().getIdProduct())));
		} else if(productTypeOfTheProduct.equals("Motherboard")) {
			cartItem.setProduct((Motherboard) motherboardConverter.toModel(motherboardService.getMotherboardById(cartItem.getProduct().getIdProduct())));
		} else if(productTypeOfTheProduct.equals("Game")) {
			cartItem.setProduct((Game) gameConverter.toModel(gameService.getGameById(cartItem.getProduct().getIdProduct())));
		}
